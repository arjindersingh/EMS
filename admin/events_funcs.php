...
SQL
    ...
    $statusColumn = $pdo->query("SHOW COLUMNS FROM events LIKE 'event_status'");
    if ($statusColumn && $statusColumn->fetch() === false) {
        $pdo->exec(
            "ALTER TABLE events ADD COLUMN event_status ENUM('Open','Suspended','Delayed','Postponed','Closed') NOT NULL DEFAULT 'Open' AFTER event_type"
        );
    }
...
function getOpenEvents(PDO $pdo): array
{
    $statement = $pdo->prepare(
        "SELECT * FROM events WHERE event_status = :event_status ORDER BY start_date DESC, event_title ASC"
    );
    $statement->execute([':event_status' => 'Open']);
...
function saveEvent(PDO $pdo, array $data): int
{
    ...
    $eventStatus = trim((string) ($data['event_status'] ?? 'Open'));
    ...
    $allowedStatuses = ['Open', 'Suspended', 'Delayed', 'Postponed', 'Closed'];
    if (!in_array($eventStatus, $allowedStatuses, true)) {
        throw new InvalidArgumentException('Please select a valid event status.');
    }
    ...
    if ($eventId > 0) {
        ...
        $pdo->prepare(
            'UPDATE events SET event_title = :event_title, event_code = :event_code, event_tagline = :event_tagline, event_theme = :event_theme, event_type = :event_type, event_status = :event_status, event_description = :event_description, start_date = :start_date, end_date = :end_date, start_time = :start_time, end_time = :end_time, venue_name = :venue_name, venue_address = :venue_address, city = :city, state = :state, country = :country, registration_required = :registration_required, registration_start_date = :registration_start_date, registration_end_date = :registration_end_date, registration_fee = :registration_fee WHERE event_id = :event_id'
        )->execute($payload);
        return $eventId;
    }

    $statement = $pdo->prepare(
        'INSERT INTO events (event_title, event_code, event_tagline, event_theme, event_type, event_status, event_description, start_date, end_date, start_time, end_time, venue_name, venue_address, city, state, country, registration_required, registration_start_date, registration_end_date, registration_fee) VALUES (:event_title, :event_code, :event_tagline, :event_theme, :event_type, :event_status, :event_description, :start_date, :end_date, :start_time, :end_time, :venue_name, :venue_address, :city, :state, :country, :registration_required, :registration_start_date, :registration_end_date, :registration_fee)'
    );
    $statement->execute($payload);
    return (int) $pdo->lastInsertId();
}
